...
def cmd_dashboard(args: argparse.Namespace) -> int:
    ...
    log_text("Starting merge-god dashboard...")

    config_path = args.config or Path("config.yaml")
    if not config_path.exists():
        log_text(f"Config file not found: {config_path}", "error")
        log_text("Run: cp config.example.yaml config.yaml", "info")
        return 1

    cmd = ["./dashboard.py", str(config_path)]
    ...
    try:
        ...
        log_text("Dashboard interrupted by user", "warning")
        return 130
    except Exception as e:
        log_text(f"Failed to start dashboard: {e}", "error")
        return 1


def cmd_scan(args: argparse.Namespace) -> int:
    """Scan PRs and sync to database (Process 1+2)"""
    log_text("Scanning and caching PR context...")

    # Build command
    cmd = ["./sync_pr_context.py"]
    ...
    try:
        ...
        if result.returncode == 0:
            log_text("PR context synced successfully", "success")
        else:
            log_text(f"Sync failed with exit code {result.returncode}", "error")
        return result.returncode
    except Exception as e:
        log_text(f"Failed to run sync: {e}", "error")
        return 1
...
def cmd_agent(args: argparse.Namespace) -> int:
    ...
    if not args.repo:
        log_text("--repo is required for agent command", "error")
        return 1
    if not args.pr:
        log_text("--pr is required for agent command", "error")
        return 1

    log_text(f"Running agent for {args.repo} PR #{args.pr}...")
    ...
    if args.db:
        cmd.extend(["--db", str(args.db)])
    if args.repo_path:
        cmd.extend(["--repo-path", str(args.repo_path)])

    try:
        result = subprocess.run(cmd, check=False)
        if result.returncode == 0:
            log_text("Agent completed successfully", "success")
        else:
            log_text(f"Agent failed with exit code {result.returncode}", "error")
        return result.returncode
    except Exception as e:
        log_text(f"Failed to run agent: {e}", "error")
        return 1


def cmd_validate(args: argparse.Namespace) -> int:
    """Validate process boundaries and data flow"""
    log_text("Validating process isolation and data flow...")

    cmd = ["./validate_process_flow.py"]
    if args.db:
        cmd.extend(["--db", str(args.db)])
    if args.repo:
        cmd.extend(["--repo", args.repo])
