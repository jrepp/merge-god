...
from models import CIStatus, PRState, RepositoryState, BranchPRState
...
class StateDatabase:
    ...
    def save_repository(self, name: str, path: str, default_branch: str | None = None) -> None:
        ...
        with self._get_connection() as conn:
            ...
            """, (name, path, default_branch, datetime.now(timezone.utc)))
    ...
    def save_pr_context(
        ...
        try:
            ...
            raise DatabaseError(f"Failed to save PR context: {e}")
    ...
    def get_latest_pr_context(self, repo_name: str, pr_number: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            raise DatabaseError(f"Failed to retrieve PR context: {e}")
    ...
    def record_agent_turn(
        ...
        with self._get_connection() as conn:
            ...
            """, (
                session_id, turn_number, role, content_type, content_preview,
                tool_uses, input_tokens, output_tokens, datetime.now(timezone.utc)
            ))
    ...
    def record_agent_error(
        ...
        with self._get_connection() as conn:
            ...
            """, (
                session_id, error_type, error_message, error_details,
                1 if is_transient else 0, retry_count, datetime.now(timezone.utc)
            ))
