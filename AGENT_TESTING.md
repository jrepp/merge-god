...
```bash
# Cache PR context
uv run python -m merge_god.cli scan --repo test-repo --pr <PR_NUMBER>
...
echo "def broken_function(): return 1/0" >> src/app.py
...
```bash
uv run python -m merge_god.cli scan --repo test-repo --pr <PR_NUMBER>
uv run python -m merge_god.cli agent --repo test-repo --pr <PR_NUMBER>
```
...
for PR in $(sqlite3 merge-god-state.db "SELECT pr_number FROM pr_context WHERE repo_name='test-repo' AND LENGTH(diff) > 100000"); do
    ...
    uv run python -m merge_god.cli agent --repo test-repo --pr $PR
done
```
...
```bash
# Capture baseline
uv run python -m merge_god.cli agent --repo test-repo --pr 100
...
# For detailed inspection, use Python
python3 << 'EOF'
from merge_god.db_operations import StateDatabase
db = StateDatabase('merge-god-state.db')
session = db.get_session_details('<session-id>')

# Print turns
for turn in session['turns']:
    print(f"\nTurn {turn['turn_number']} ({turn['role']}):")
    print(f"  Type: {turn['content_type']}")
    print(f"  Tools: {turn['tool_uses']}")
...
# Re-sync if missing
uv run python -m merge_god.cli scan --repo test-repo --pr 123
```
...
- [TESTING.md](TESTING.md) - Complete testing workflow
