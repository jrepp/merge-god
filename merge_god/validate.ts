 ...
 *
 * Mirrors `PRContext` in merge_god/agents/claude_agent.py, which has no TS
 * port yet. Defined locally so this module compiles standalone.
 */
interface PRContext {
  pr_number: number;
  title: string;
...
/**
 * Construct a PRContext from scanner data structures.
 *
 * Mirrors `PRContext.from_dict` in merge_god/agents/claude_agent.py. Throws if
 * `pr_details.number` is not a number.
 */
function createPRContextFromDict(
  ...
  return {
    ...
    head_branch: typeof prDetails["headRefName"] === "string" ? prDetails["headRefName"] : "",
    base_branch: typeof prDetails["baseRefName"] === "string" ? prDetails["baseRefName"] : "main",
    ...
    conflicting_files: Array.isArray(conflicts["conflicting_files"]) ? conflicts["conflicting_files"] : [],
    has_failing_ci: typeof failed === "number" ? failed > 0 : false,
    failing_checks: Array.isArray(ciStatus["failed_checks"]) ? ciStatus["failed_checks"] : [],
    review_comments: Array.isArray(prContext["review_comments"]) ? prContext["review_comments"] : [],
    ...
    commit_examples: typeof prContext["commit_examples"] === "string" ? prContext["commit_examples"] : "",
    labels: Array.isArray(prDetails["labels"]) ? (prDetails["labels"] as string[]) : [],
    ci_checks: ciStatus,
    review_decision: typeof prDetails["reviewDecision"] === "string" ? prDetails["reviewDecision"] : null,
  };
}
