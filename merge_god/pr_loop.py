...
if TYPE_CHECKING:
    from agents import PRAgent, PRContext, PRProcessingCallbacks
        ...
        if not isinstance(details, dict):
            log_json(
                "get_pr_details",
                {
                    "action": "parse_error",
                    "pr_number": pr_number,
                    "error": "Expected dict, got " + type(details).__name__,
                },
            )
            return {}
    except json.JSONDecodeError as e:
...
def build_pr_prompt(
    ...
    if commits:
        ...
        for commit in commits[-10:]:  # Last 10 commits
            ...
            short_sha = (
                sha[:GIT_SHORT_SHA_LENGTH]
                if sha and len(sha) >= GIT_SHORT_SHA_LENGTH
                else (sha if sha else "unknown")
            )
...
def build_review_prompt(
    ...
    prompt_parts.extend(
        [
            ...
            "",
            "```diff",
            diff[:MAX_DIFF_DISPLAY_SIZE]
            if len(diff) > MAX_DIFF_DISPLAY_SIZE
            else diff,  # Cap at ~100KB to avoid overwhelming
            "```",
            "",
            "## Review Guidelines",
            "",
...
async def process_pr_async(
    ...
    if db and repo_name:
        try:
            ...
            db.save_pr_context(
                repo_name,
                pr_number,
                cast("dict[str, Any]", pr_details),
                cast("dict[str, Any]", pr_context_dict),
            )
            log_json(
                "process_pr",
                {
    ...
    try:
        # TypedDict is compatible with dict[str, Any], cast for mypy
        pr_context = PRContext.from_dict(
            cast("dict[str, Any]", pr_details), cast("dict[str, Any]", pr_context_dict)
        )
        log_json(
            "process_pr",
            {
