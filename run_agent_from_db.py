...
async def run_agent_from_db(
    ...
    callbacks = PRProcessingCallbacks(
        pr_number=pr_number,
        log_json=log_json,
        send_notification=lambda *args, **kwargs: True,  # Disable notifications
    )
    ...
    try:
        result = await agent.process_pr_streaming(  # type: ignore[assignment]
        ...
        if session_id:
            try:
                db.update_agent_session(
                    session_id=session_id,
                    status="completed" if result.success else "failed",  # type: ignore[attr-defined]
                    success=result.success,  # type: ignore[attr-defined]
                    tasks_total=len(result.tasks),  # type: ignore[attr-defined]
                    tasks_completed=len([t for t in result.tasks if t.status == "completed"]),  # type: ignore[attr-defined]
                    tasks_failed=len([t for t in result.tasks if t.status == "failed"]),  # type: ignore[attr-defined]
                    actions_total=len(result.actions),  # type: ignore[attr-defined]
                )
            except Exception as e:
                log_json(
                    "agent_from_db",
        ...
        log_json(
            "agent_from_db",
            {
                ...
                "session_id": session_id,
                "success": result.success,  # type: ignore[attr-defined]
                "duration": result.duration,  # type: ignore[attr-defined]
                "tasks_total": len(result.tasks),  # type: ignore[attr-defined]
                "tasks_completed": len([t for t in result.tasks if t.status == "completed"]),  # type: ignore[attr-defined]
                "tasks_failed": len([t for t in result.tasks if t.status == "failed"]),  # type: ignore[attr-defined]
                "actions_taken": len(result.actions),  # type: ignore[attr-defined]
                "mode": mode,
            },
        )

        return result.success  # type: ignore[return-value,attr-defined]
